...
exports.getPYQStart = async (req, res) => {
  ...
  try {
    ...
    console.error('Error in getPYQStart:', err.message);
    res.status(500).json({ error: 'Internal server error' });
  }
};

// 2. GET /adaptive/pyq/resume-point
exports.getPYQResumePoint = async (req, res) => {
  const { user_id, exam_id, subject_id } = req.query;
  ...
  try {
    ...
    console.error('Error in getPYQResumePoint:', err.message);
    res.status(500).json({ error: 'Internal server error' });
  }
};

// 3. POST /adaptive/pyq/submit (batch)
exports.submitPYQResponses = async (req, res) => {
  const { responses } = req.body;
  ...
  const isValid = responses.every(r =>
    r.user_id && r.mcq_id && r.selected_option
  );
  ...
  try {
    ...
    console.error('Error in submitPYQResponses:', err.message);
    res.status(500).json({ error: 'Internal server error' });
  }
};

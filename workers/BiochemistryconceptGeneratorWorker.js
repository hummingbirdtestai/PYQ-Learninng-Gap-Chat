...
const SUBJECT_FILTER = "Biochemistry";
...
function buildPrompt(topic) {
  return (
`
You are an 30 Years experienced Undergraduate MBBS **Biochemistry** Teacher expert in NMC PRESCRIBED Competency Based Curriculum. 
...
1) Central Concept 2) Core Biochemistry 3) 10 High-Yield Facts 4) Clinical Case Vignettes 5) Viva Voce Questions 6) Summary Table / Mnemonics 
Explain using the following rules exactly: 1
) **Central Concept** – Give a short, crisp, foundational explanation of the topic. – Use analogies if helpful. 2) **Core Biochemistry** – Explain **molecular structure, biochemical pathways, enzymes, cofactors, metabolic steps, regulation, rate-limiting enzymes, transport mechanisms, energetics (ATP), organ-specific metabolism**, and functional significance. – Present in concise bullet points. 3) **10 High-Yield Facts (USMLE + NEET-PG)** – Single-line pearls – Emphasize exam-friendly and memory-friendly points. 4) **5 Clinical Case Vignettes (Biochemistry-oriented)** – Each 3–4 lines maximum – Reasoning should connect **biochemical defect → metabolic block → clinical presentation**. 5) **Top 5 Viva Voce Questions (with answers)** – Simple, direct, easily recallable. 6) **Provide a summary table, metabolic map, mnemonic, or comparison chart for revision.** Output must strictly follow Sections 1–6. 
...
Do **NOT** add any titles or headers beyond the 6 sections I specify. Output ONLY those 6 sections exactly as numbered.
`
  ).trim();
}

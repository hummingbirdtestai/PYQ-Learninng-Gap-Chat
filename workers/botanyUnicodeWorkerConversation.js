// workers/botanyUnicodeWorkerConversation.js
require("dotenv").config();
const { supabase } = require("../config/supabaseClient");
const openai = require("../config/openaiClient");

// ---------- Settings ----------
const MODEL        = process.env.BOTANY_CONVERSATION_UNICODE_MODEL || "gpt-5-mini";
const LIMIT        = parseInt(process.env.BOTANY_CONVERSATION_UNICODE_LIMIT || "200", 10);
const BATCH_SIZE   = parseInt(process.env.BOTANY_CONVERSATION_UNICODE_BATCH_SIZE || "10", 10);
const SLEEP_MS     = parseInt(process.env.BOTANY_CONVERSATION_UNICODE_LOOP_SLEEP_MS || "500", 10);
const LOCK_TTL_MIN = parseInt(process.env.BOTANY_CONVERSATION_UNICODE_LOCK_TTL_MIN || "15", 10);
const SUBJECT_FILTER = "Botany"; // 🔒 subject filter
const WORKER_ID    = process.env.WORKER_ID || `botanyunicodeworkerconversation-${process.pid}-${Math.random().toString(36).slice(2,8)}`;
  ...
  return [
    {
      role: "system",
      content: `
You are a senior 30+ yrs exp NEET Botany Teacher.
...
// ---------- Main ----------
(async function main() {
  console.log(`🌿 Botany Unicode Conversation Worker ${WORKER_ID} | model=${MODEL} | claim=${LIMIT} | batch=${BATCH_SIZE}`);
  while (true) {
    try {

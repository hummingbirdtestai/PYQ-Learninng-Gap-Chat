...
const WORKER_ID    = process.env.WORKER_ID || `concept-phase-${process.pid}-${Math.random().toString(36).slice(2,8)}`;
...
// ---------- Main ----------
(async function main() {
  console.log(`🧬 Concept Phase Worker ${WORKER_ID} | model=${MODEL} | claim=${LIMIT} | batch=${BATCH_SIZE}`);
  while (true) {
    try {

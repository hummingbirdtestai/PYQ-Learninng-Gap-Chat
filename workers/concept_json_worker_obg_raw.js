...
async function processRow(row) {
  ...
  try {
    ...
    console.error("❌ JSON parse failed, retrying once:", row.id);
    raw = await callOpenAI(prompt);
    parsed = safeParseJson(raw);
  }

  if (!Array.isArray(parsed) || parsed.length !== 10) {
    throw new Error("❌ Output is not an array of 10 concepts");
  }
...
// ─────────────────────────────────────────────
(async function main() {
  console.log(`🧠 CONCEPT JSON WORKER STARTED | ${WORKER_ID} | model=${MODEL}`);

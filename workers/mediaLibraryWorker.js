...
const LIMIT        = parseInt(process.env.MCQ_LIMIT || "100", 10);
...
const WORKER_ID    = process.env.WORKER_ID || `media-${process.pid}-${Math.random().toString(36).slice(2,8)}`;
...
function buildPrompt(conceptJson) {
  ...
  return `
You are a 40-year experienced NEET Chemistry teacher.  

From the Concept + Explanation below, create **3 Google Image searches** and **3 YouTube video searches**, each with a short description.  
...
// ---------- Main Loop ----------
(async function main() {
  console.log(`🧵 MediaLibrary Worker ${WORKER_ID} | model=${MODEL} | claim=${LIMIT} | batch=${BATCH_SIZE}`);

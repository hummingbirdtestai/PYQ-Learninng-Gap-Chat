...
const WORKER_ID    = process.env.WORKER_ID || `mentor-intro-flashcard-${process.pid}-${Math.random().toString(36).slice(2,8)}`;

// ---------- Prompt ----------
function buildPrompt(concept) {
  return `
You are a passionate NEET-PG mentor with 30 years’ teaching experience.  
Given a phase_json (concept), write a 3-sentence empathetic motivational intro in simple dialogue style that sounds like a friendly teacher.  
...
async function claimRows(limit) {
  ...
  const { data: locked, error: e2 } = await supabase
    .from("flashcard_raw")
    .update({ mentor_reply_lock: WORKER_ID, mentor_reply_lock_at: new Date().toISOString() })
    .in("id", ids)
    .is("mentor_reply", null)
    .is("mentor_reply_lock", null)
    .select("id, concept");
...
// ---------- Main ----------
(async function main() {
  console.log(`🧵 Mentor Intro Flashcard Worker ${WORKER_ID} | model=${MODEL}`);
  while (true) {
    try {

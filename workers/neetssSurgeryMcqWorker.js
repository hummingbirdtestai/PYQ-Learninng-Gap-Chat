...
const MODEL =
  process.env.NEETSS_SURGERY_MCQ_MODEL ||
  "gpt-5.6-terra";

const PICKUP_LIMIT = parseIntegerEnv(
  "NEETSS_SURGERY_MCQ_LIMIT",
  50,
  1,
  100
);
...
const LOOP_SLEEP_MS = parseIntegerEnv(
  "NEETSS_SURGERY_MCQ_LOOP_SLEEP_MS",
  1000,
  250,
  60000
);
...
const LOCK_TTL_MIN = parseIntegerEnv(
  "NEETSS_SURGERY_MCQ_LOCK_TTL_MIN",
  120,
  5,
  1440
);
...
const API_RETRIES = parseIntegerEnv(
  "NEETSS_SURGERY_MCQ_API_RETRIES",
  2,
  0,
  5
);
...
const MAX_OUTPUT_TOKENS = parseIntegerEnv(
  "NEETSS_SURGERY_MCQ_MAX_OUTPUT_TOKENS",
  18000,
  5000,
  30000
);
...
const WORKER_ID =
  process.env.NEETSS_SURGERY_MCQ_WORKER_ID ||
  `neetss-surgery-mcq-${process.pid}-${Math.random()
    .toString(36)
    .slice(2, 8)}`;
...
OUTPUT Return ONLY valid JSON CONTAINNING EACH MCQ as a object in the JSON : { "Stem": "...", "A": "...", "B": "...", "C": "...", "D": "...", "Correct Answer": "A", "Explanation": { "Correct Answer Summary": "...", "Diagnostic Pathway": ["...", "...", "..."], "Why the Other Options Fail": { "B": "...", "C": "...", "D": "..." }, "Examiner's Trap": "..." } } IMPORTANT JSON REQUIREMENT: Return exmcqs": [ { "Stem": "...", "A": "...", "B": "...", "C": "...", "D": "...", "Correct Answer": "A", "Explanation": { "Correct Answer Summary": "...", "Diagnostic Pathway": ["...", "...", "..."], "Why the Other Options Fail": { "B": "...", "C": "...", "D": "..." }, "Examiner's Trap": "..." } } ] } The mcqs": "..." }, "Examiner's Trap": "..." } } ] } The mcqs array must contain exactly 10 MCQ objects. Do not include Markdown fences or any text outside JSON.
`.trim();
...
function buildUserInput(row) {
  return [
    ...
    "",
    "SUPPLIED NEET-SS SURGERY RAPID-REVISION NOTES JSON:",
    serializeJson(row[INPUT_COL]),
    "",
    "Generate exactly 10 MCQs and return one valid JSON object."
  ].join("\n");
}
...
function validateGeneratedOutput(rawOutput) {
  ...
  if (!Array.isArray(parsed.mcqs)) {
    throw new Error(
      "Generated output is missing mcqs array"
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Generated output contains ${parsed.mcqs.length} MCQs; exactly ${REQUIRED_MCQ_COUNT} required`
    );
  }
    ...
    parsed.mcqs.map(
      (mcq, index) => {
        ...
        if (
          ...
          throw new Error(
            `MCQ ${number} Stem has ${stemWordCount} words; required range is 30–40`
          );
        }
        ...
        for (
          ...
          if (
            ...
            throw new Error(
              `MCQ ${number} option ${letter} has ${optionWordCount} words; required range is 2–5`
            );
          }
        }
        ...
        if (
          ...
          throw new Error(
            `MCQ ${number} contains duplicate options`
          );
        }
        ...
        if (
          ...
          throw new Error(
            `MCQ ${number} has invalid Explanation`
          );
        }
        ...
        if (
          ...
          diagnosticPathway.length < 3 ||
          diagnosticPathway.length > 6
        ) {
          throw new Error(
            `MCQ ${number} Diagnostic Pathway must contain 3–6 steps`
          );
        }

        const normalizedPathway =
          diagnosticPathway.map(
            (step, stepIndex) =>
              requireNonEmptyString(
                step,
        ...
        if (
          ...
          throw new Error(
            `MCQ ${number} has invalid Why the Other Options Fail`
          );
        }
        ...
        if (
          ...
          throw new Error(
            `MCQ ${number} failure keys must be exactly ${expectedFailureKeys.join(", ")}`
          );
        }
        ...
        for (
          ...
          normalizedFailures[letter] =
            requireNonEmptyString(
              failures[letter],
              `MCQ ${number} failure explanation ${letter}`
            );
        }
        ...
        if (seenStems.has(stemKey)) {
          throw new Error(
            `MCQ ${number} duplicates another Stem`
          );
        }
...
async function generateMcqs(row) {
  ...
  for (
    ...
    try {
      ...
      /*
       * Retry API failures and model-output
       * validation failures. Validation failures
       * are common when generating 10 complex
       * structured questions.
       */
      const shouldRetry =
        isRetryableApiError(error) ||
        /invalid JSON|exactly 10|required range|duplicate|failure keys|Diagnostic Pathway|non-empty string|missing mcqs/i.test(
          getErrorText(error)
        );
...
async function processRow(row) {
  ...
  try {
    ...
    console.log(
      `✅ Completed | ${row.subject} | ${row.serial_number} | MCQs=${generatedOutput.mcqs.length}`
    );
...
async function main() {
  console.log(
    `🚀 NEET-SS SURGERY MCQ WORKER STARTED: ${WORKER_ID}`
  );

  console.log(
    `⚙️ Model=${MODEL} | Pickup=${PICKUP_LIMIT} | Concurrent=${BATCH_SIZE} | MCQs/topic=${REQUIRED_MCQ_COUNT} | Max output=${MAX_OUTPUT_TOKENS}`
  );
...
main().catch((error) => {
  console.error(
    "❌ Fatal NEET-SS Surgery MCQ worker error:",
    error
  );

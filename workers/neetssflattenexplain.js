...
const COURSE_ID =
  "31860d69-156b-4db8-a41d-77e223204b8d";
...
const MODEL =
  process.env.NEETSS_MED_EXPLANATION_MODEL ||
  "gpt-5-mini";

const PICKUP_LIMIT = parseIntegerEnv(
  "NEETSS_MED_EXPLANATION_LIMIT",
  20,
  1,
  100
);
...
const BATCH_SIZE = parseIntegerEnv(
  "NEETSS_MED_EXPLANATION_BATCH_SIZE",
  5,
  1,
  20
);
...
const LOOP_SLEEP_MS = parseIntegerEnv(
  "NEETSS_MED_EXPLANATION_LOOP_SLEEP_MS",
  1000,
  250,
  60000
);
...
const LOCK_TTL_MIN = parseIntegerEnv(
  "NEETSS_MED_EXPLANATION_LOCK_TTL_MIN",
  120,
  5,
  1440
);
...
const API_RETRIES = parseIntegerEnv(
  "NEETSS_MED_EXPLANATION_API_RETRIES",
  2,
  0,
  5
);
...
const WORKER_ID =
  ...
  `neetss-med-explanation-${process.pid}-${Math.random()
    .toString(36)
    .slice(2, 8)}`;
...
// ─────────────────────────────────────────────
// BUILD INPUT
// Input is question only, with reference context.
// ─────────────────────────────────────────────
...
// Restricted to the specified course.
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// LOCK ONE ROW
// Lock timestamp is the ownership token.
// ─────────────────────────────────────────────
...
async function saveSuccess(
  ...
  if (!data?.length) {
    throw new Error(
      "Save rejected because the lock changed or explanation already exists"
    );
  }
}
...
async function main() {
  console.log(
    `🚀 NEET SS MEDICINE EXPLANATION WORKER STARTED: ${WORKER_ID}`
  );
...
main().catch((error) => {
  console.error(
    "❌ Fatal NEET SS Medicine explanation worker error:",
    error
  );

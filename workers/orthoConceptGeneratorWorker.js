...
const SUBJECT_FILTER = "Radiodiagnosis";
...
function buildPrompt(topic) {
  return (
`
You are an 30 Years experienced Undergraduate MBBS **Radiology** Teacher expert in NMC PRESCRIBED Competency Based Curriculum. 
...
2) **Core Radiology**  
   – Explain **basic imaging physics (X-ray, CT, MRI, USG), radiation safety, contrast agents, imaging signs, common views/planes, interpretation principles, normal vs abnormal radiographic anatomy, pattern recognition (lung, abdomen, bones, neuroimaging), emergency imaging (FAST, CT trauma), common pathologies (pneumonia, effusion, fractures, intracranial bleed, obstruction), artifacts, modality selection (which test when), and limitations**, with clinical reasoning.  
...
4) **5 Clinical Case Vignettes (Radiology-oriented)**  
   – Each 3–4 lines maximum  
   – Reasoning should connect **symptom → imaging modality chosen → key finding → likely diagnosis**.

5) **Top 5 Viva Voce Questions (with answers)**  
   – Simple, direct, easily recallable.

6) **Provide a summary table, imaging sign chart, modality comparison table, radiation dose table, contrast agent summary, or mnemonic for revision.**

Output must strictly follow Sections 1–6.  
Give the output **strictly in Markdown code blocks** with Unicode symbols.  
In the output, explicitly **bold and italicize** all important key words, imaging signs, modalities, pathologies, and headings for emphasis using proper Markdown (e.g., *bold, italic*).  

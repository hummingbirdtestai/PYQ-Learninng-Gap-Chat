// workers/unicodeMediaLibraryWorker.js
...
const SUBJECT_FILTER = process.env.MEDIA_UNICODE_SUBJECT || null;
const WORKER_ID    = process.env.WORKER_ID || `unicode-media-${process.pid}-${Math.random().toString(36).slice(2,8)}`;

// ---------- Prompt ----------
function buildPrompt(mediaJson) {
  const compact = JSON.stringify(mediaJson);
  return `
...
async function claimRows(limit) {
  ...
  let q = supabase
    .from("concepts_vertical")
    .select("vertical_id, media_library")
    .not("media_library", "is", null)
    .is("media_library_unicode", null)
...
async function processRow(row) {
  const prompt = buildPrompt(row.media_library);
...
// ---------- Main ----------
(async function main() {
  console.log(`🧵 Unicode MediaLibrary Worker ${WORKER_ID} | model=${MODEL} | claim=${LIMIT} | batch=${BATCH_SIZE}`);
  while (true) {
    try {
